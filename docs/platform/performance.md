...
per-message line that names tools already loaded for this turn sit on the
latest user message, so a changing promotion list does not bust the prefix.
The trailing channel contract stays last. Tool definitions are emitted in
deterministic name order. On multi-hop turns, cached input tokens should be
at least 60% of input tokens. History is trimmed before the latest user
message is dropped; memory and cumulative tool results have explicit ceilings
and report truncation. An attachment must keep that assembled latest-message
text; replacing it with the raw composer text drops memory and retrieval.

Insight blocks are built after the answer is written, on the platform chat
model, and do not sit inside the tool loop. A refusal does not generate
them. Playground visual cards are saved after the assistant message and may
arrive on the stream up to the follow-up chip deadline; they must not delay
`persisted`. Channel turns still return insights with the reply, bounded at
6 seconds so a slow insights model cannot hold delivery. This follows the
same instrument-before/after discipline and `EdgeRuntime.waitUntil`
stale-while-revalidate boundary used by the read paths below.
...
| Rule | Rationale |
|---|---|
| **CRM lists are live Qobrix reads (ADR-051)** | Leads and Pipeline list/board tabs call Qobrix `/opportunities` under the caller's browser session for **all** scopes. Qobrix ACL is the row filter; MSA does not push down owner scope on the live fetch. App DB is the write/copy overlay (MSA stage, contracts, app-authored rows). No mirror, no App-DB list fallback — missing Qobrix session → hard fail. Header Refresh invalidates queries; admin "Sync all" walks the copy store. |
| **Two-source merge uses bounded-overfetch (ADR-052)** | Keep small legs (app-authored + overlay corrections) fully in memory; page only the Qobrix leg with window `[max(0, o − nSmall), o + s)`. Overfetch ≤ `s + nSmall`. Total order `(created, id)`. Never prefetch a board with `BOARD_MAX_ROWS` and slice in memory. Contract-derived columns (`contracting`/`payment`) invert who pages but still gate visibility through Qobrix `id in [...]`. |
| **Batch id lookup uses `field in [...]` only (chunk ≤ 100)** | NEVER build `(id=="a" or id=="b" …)`. Measured 2026-09-02 on `/properties`: OR of 10 ids succeeds; OR of 20+ returns **HTTP 504** (even with `fields=['id']`, no media). Native `id in ['…']` returns 30 and 100 ids in one fast page, including with `media=true`. Same `in` form works on `/opportunities`, `/contacts`, `/agents`, `/projects`. Helper: MSA `idInClause` / `ID_IN_CHUNK=100` in `supabase/functions/_shared/qobrix-search.ts` (client twin in `src/lib/sales/qobrixDsl.ts`). **Only for identifiers and codes** matching `[A-Za-z0-9_-]{1,64}` — the sanitizer silently drops anything else (spaces, punctuation), which would shrink lead buckets like Disqualified (`Invalid Request`) and reroute (`For distribution`). Free-text / human-label lists must use a normal string quoter (`dslQuote` → `"…"`); inside `in [...]` both quote styles work, while `==` requires double quotes. Aligns ADR-050/052 id-hydration chunks (was ≤40/≤50; native `in` supports 100). |
| **Every Qobrix fetch uses the caller's token** | Browser-held session only; no `QOBRIX_SHARE_*` on reads. |
| **RLS policies must inline row predicates; scalar-subquery wrap parameterless helpers only** | Wrapping `(SELECT can_read_claimed_row(row_cols…))` does **not** hoist an InitPlan when the helper takes row columns — Postgres builds a per-row `SubPlan`. Worse: `can_read_claimed_row` is `SECURITY DEFINER`, so it cannot be inlined at all. After Phase 4 promotion (~43k leads, ~46k contacts), a global-admin `count(*)` on `leads where status='new'` measured **10,028 ms** (`SubPlan 2 … loops=43359`) against an **8 s** `authenticated` `statement_timeout`. **Fix:** inline the predicate into the policy — caller identity as `(SELECT get_current_user_id())` / `(SELECT get_active_scope())` (InitPlan, once); row terms as bare column comparisons so indexes apply. Same query after inlining: **2.4 ms**. Parameterless helpers (`get_current_tenant_id`, `_is_call_center_or_admin`) still benefit from `(SELECT …)`. Confirm with `EXPLAIN ANALYZE` before/after. |
...
| **Best-effort upstream enrichment must not gate the response** | Optional id-batch enrichment (`id in [...]`) sits behind a soft deadline. On expiry return what is resolved; never `catch {}` a fan-out of 12s `qFetch` aborts. |
