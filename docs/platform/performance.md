...
| **Prefer an App-DB opportunity mirror for the pipeline board** | `qobrix_opportunity_mirror` (cron-synced) serves board reads when fresh; live Qobrix is the fallback. Filters, sort, and per-column counts become indexed Postgres queries. Keep MSA `opportunities` as copy-on-write SoT — do not dump mirrored rows into it. |
