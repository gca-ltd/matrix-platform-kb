...
| **Batch id lookup uses `field in [...]` only (chunk ≤ 100)** | NEVER build `(id=="a" or id=="b" …)`. Measured 2026-09-02 on `/properties`: OR of 10 ids succeeds; OR of 20+ returns **HTTP 504** (even with `fields=['id']`, no media). Native `id in ['…']` returns 30 and 100 ids in one fast page, including with `media=true`. Same `in` form works on `/opportunities`, `/contacts`, `/agents`, `/projects`. Helper: MSA `idInClause` / `ID_IN_CHUNK=100` in `supabase/functions/_shared/qobrix-search.ts` (client twin in `src/lib/sales/qobrixDsl.ts`). Aligns ADR-050/052 id-hydration chunks (was ≤40/≤50; native `in` supports 100). |
