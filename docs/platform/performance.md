...
| **Render the page, then refresh the page's rows** | Leads and Pipeline lists query App DB with filter/sort/pagination in SQL (PostgREST `.range()` + `count: 'exact'`). After the list settles, fire `qobrix-user-refresh` `mode=slice` with the visible rows' `external_qobrix_id`s — never replay the list query upstream (ordering diverges). Chunk id-list Qobrix searches at 50 ids; skip ids inside a `qobrix_last_synced_at` TTL. Invalidate only that view's React Query key. Full-collection walks are admin-only. Pipeline stage chips use a grouped aggregate RPC (`pipeline_board_groups`, SECURITY INVOKER) instead of prefetching thousands of rows. |
