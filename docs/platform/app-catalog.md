...
> **Matrix Datacore** — `gca-ltd/matrix-datacore`, Cursor-managed (not Lovable, not github-watcher). Supabase Pro project [`zcajghoohycimpubufsy`](https://supabase.com/dashboard/project/zcajghoohycimpubufsy) in `eu-central-1` (Frankfurt). Schemas: `raw` (Qobrix photograph, service_role only), `insights`, `compliance` (initiative 005), `referral` (initiative 006). Qobrix access via API/MCP only (no direct DB / no true CDC). EU-only personal data; RUSIR/amoCRM must not land here. Ingest worker is `sync/` CLI, **on-demand only** (no pm2/cron cadence — Qobrix API cost + CRM load). **KB divergence:** this path covers 005/006 ingest without Databricks (ADR-004 still describes Databricks as DWH — ADR update is a follow-up). Design doc: `matrix-datacore/docs/005-006-qobrix-ingest.md`.
...
> **Matrix Sales Automation** — `gca-ltd/matrix-sales-automation` (was `sharpsir-group/matrix-qobrix-sales-automation-v1-0`, archived as `…-v1-0-superseded` on 2026-08-20). Qobrix-strangler sales CRM (Domain-Specific). App DB / Edge Functions on Supabase project `rpoeezssicpzexarmwqq`. Two equal github-watcher targets: `gca-ltd/matrix-sales-automation@main` → `/msa-staging-main/` (business user) and `@cdto` → `/msa-staging-cdto/` (Cursor). Legacy `/qobrix-v1.0/` 301s to `/msa-staging-main/`. **Hungary variant:** `gca-ltd/matrix-sales-automation-hungary`, app DB `ykgyzqnuqpwasxvesxva`, watcher path `/msa-hungary-staging-main/`. Do not confuse with the thin-client sibling `-rls` above (`/qobrix-rls/`, project `ycbwgnihbrqammkgngum`).
