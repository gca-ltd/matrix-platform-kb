...
> **Matrix Datacore** — `gca-ltd/matrix-datacore`, Cursor-managed (not Lovable, not github-watcher). Supabase Pro project [`zcajghoohycimpubufsy`](https://supabase.com/dashboard/project/zcajghoohycimpubufsy) in `eu-central-1` (Frankfurt). Schemas: `raw` (Qobrix photograph), `core` (materialized normalized model), `insights` / `compliance` (005) / `referral` (006). **`public` is the curated agent contract** (`object_catalog`, `contact`, `opportunity`, `activity`, `alert_candidate`, `prediction`, …) — not raw mirrors. No `anon`/`authenticated` grants; ingest writes `raw`; after sync run `core.refresh_all()`. Hybrid naming: RESO snake_case for Contact/Member; project-flavour for Opportunity/Contract/activity (intentional divergence from ADR-029 CDL decomposition — analytical snapshot, not CDL write). Qobrix via REST (read-only); **nightly incremental** via bastion `scripts/nightly.sh` (cron Europe/Nicosia 02:00, not pm2). **MCP:** Edge Function `datacore-mcp` Mode B bearer — `https://zcajghoohycimpubufsy.supabase.co/functions/v1/datacore-mcp` (overview/alerts/predictions/freshness/catalog/lookup; no live DeepSeek). Docs: `matrix-datacore/docs/agent-data-model.md`, `docs/demo-005-006.md`. **KB divergence:** bypasses Databricks (ADR-004 pending); Opportunity/Contract not RESO-decomposed.
>
> **Sharp SIR Charts** — `gca-ltd/sharpsir-charts-mcp-server`, public MIT Chart.js rendering MCP. Local stdio is the default; hosted Streamable HTTP is operator-managed at `https://intranet.sharpsir.group/charts/mcp` on port 3512 with static output under `/charts/o/`. The caller supplies JSON data; the server has no data connector. The default `sothebys` design is session-scoped and agent-configurable, with Sharp Matrix, mono-print and neutral references. See [`matrix-mcp-server.md`](matrix-mcp-server.md#sharp-sir-charts).
...
> **Matrix Sales Automation (Cyprus staging)** — `gca-ltd/matrix-sales-automation` (was `sharpsir-group/matrix-qobrix-sales-automation-v1-0`, archived as `…-v1-0-superseded` on 2026-08-20). Qobrix-strangler sales CRM (Domain-Specific). App DB / Edge Functions on Supabase project `rpoeezssicpzexarmwqq`. OAuth `client_id` `Dk4cIY3~VvwYYgFIU.2gCdAfewWb34AZ` (portal title **Matrix Sales Automation**, subtitle *Staging — pipeline, leads & listings*, `display_order` 33). Two equal github-watcher targets: `gca-ltd/matrix-sales-automation@main` → `/msa-staging-main/` (business user; portal `app_url`) and `@cdto` → `/msa-staging-cdto/` (Cursor). Redirect URIs include both `/msa-staging-main/auth/callback` and `/msa-staging-cdto/auth/callback`. Legacy `/qobrix-v1.0/` 301s to `/msa-staging-main/`. Inventory navigation is a single **Listings** page (`/listings`); legacy `/properties` and `/projects` redirect there (2026-09). **Sharp SIR portal access** (migration `20260824110000_msa_staging_main_portal_and_sales_access.sql`): Broker, Senior Broker, Area Manager, Team Leader, Sales Manager, Sales Director, Call Centre, and CORE Team have the client in `apps_allowed`; sales/CORE roles have `pages/actions='*'` (Call Centre keeps leads/approvals/analytics). **Cyprus office teams** (32-person roster → `CSIR Sales Paphos` / `Limassol` / `Larnaca`; leads Iness Karayianni, Olga Khokhlova, Liza Kazares): `20260824130000_cy_roster_office_group_align.sql` — see [security-model.md](security-model.md). Do not confuse with the thin-client sibling `-rls` above (`/qobrix-rls/`, project `ycbwgnihbrqammkgngum`).
>
> **Matrix Sales Automation — Hungary** — `gca-ltd/matrix-sales-automation-hungary`. Lovable + Cursor co-authored Domain-Specific sales CRM for the Budapest office. **App DB / Edge Functions:** Supabase project `ykgyzqnuqpwasxvesxva` (`verify_jwt: false` on EFs). **Deploy:** github-watcher `gca-ltd/matrix-sales-automation-hungary@main` → `https://intranet.sharpsir.group/msa-hungary-staging-main/` (`/opt/bitnami/apache/htdocs/msa-hungary-staging-main`). SSO redirect: `https://intranet.sharpsir.group/msa-hungary-staging-main/auth/callback`. **Listings catalog:** read-only from **Matrix Digital Storefront 2.0 Hungary** (`bpaxqtxaysolzaeguwvg`) via anonymous PostgREST in the browser — see [data-distribution-and-stewardship.md § Hungary MSA](../architecture/data-distribution-and-stewardship.md#hungary-msa--storefront-20-read-path). **Website leads:** `storefront-leads-ingest` EF reads `property_inquiries` server-side and upserts App-DB `leads`. **Qobrix removed (2026-08-25):** no `qobrix-*` edge functions; App DB is system of record for pipeline/leads; off-market listings are App-DB-authored. **Parity port (2026-09):** Cyprus MSA sales-process packages (matching workspace, pipeline stage gates, shortlists/share, lead triage, off-market/land, viewing rollup, sales methodology page) forward-ported without reintroducing Qobrix read paths. Reports/analytics Qobrix legs and contract-detail Qobrix UI remain deferred.

> **Matrix Sales Automation — Hungary sandbox** — Lovable-only clone for isolated work (SR000537). Twin repo `gca-ltd/remix-of-matrix-sales-automation-hungary`, Lovable project `c2692ae4-21e6-4a35-9273-a787d3d3a279` (`https://matrix-sales-automation-hungary-sandbox.lovable.app`). **App DB / Edge Functions:** `mstvgnekgeoftwajxscv` (cloned from Hungary `ykgyzqnuqpwasxvesxva`; ES256 via Third-Party Auth — do **not** HS256-bind `jwt_secret_name`). OAuth client `us5vHZWN7WHiT0IkSXzACl~oeuM_If2t` (`show_in_portal=false`; Lovable redirect URIs only; no github-watcher / intranet subpath). Listings catalog remains the shared Storefront 2.0 Hungary read path. Do not copy `STOREFRONT_SERVICE_ROLE_KEY` onto the sandbox (website-lead ingest stays on live).
...
>
> **Digital Employees** (SSO title **AI Agents**) — `gca-ltd/matrix-digital-employees`, remixed from `matrix-apps-template-2-2`. Auto-deployed by `github-watcher` on push to `main` (config key `gca-ltd/matrix-digital-employees`, secret env `WEBHOOK_SECRET_DIGITAL_EMPLOYEES`, Apache path `/digital-employees/`). Production URL: `https://intranet.sharpsir.group/digital-employees/`. OAuth client `eXN8sUlGHJGHkcizgZNC0bdvOhSfbtst` — intranet redirect `https://intranet.sharpsir.group/digital-employees/auth/callback` (migration `20260818100000`). App DB / EFs on Supabase project `mihslqjjclbrqelnjjpb`, which is **Lovable Cloud managed** (Lovable project `517d659a-ad9c-45a5-8931-e126f2e7c5f1`). The Supabase CLI and Management API return 403 from this host — migrations and edge-function deploys go through the Lovable MCP (`user-lovable-mde`: `query_database` / `send_message`), not `supabase functions deploy`. See repo `AGENTS.md` § Lovable Cloud. The leftover GitHub Actions rsync `deploy.yml` is unused (no `DEPLOY_*` secrets), same as ITSM. **Playground conversation links** are tenant-wide read/write (anyone signed in to the same tenant who holds the URL can open and continue the thread; list / rename / star / delete stay owner-scoped) — see [app-template.md](app-template.md#digital-employees--playground-conversation-links). **Teams channel:** see [teams-channel.md](teams-channel.md). **Public Conversations API** (`/functions/v1/converse`, OpenAPI 1.1.0): stateful chat, `stateless` one-shots, `/transcribe`, `/suggest` (broker drafts), `/translate` — the in-platform replacement surface for Matrix Comms' HumaticAI RAGChat usage (SR000518; cutover map in the app repo `docs/public-api-comms-integration.md`). **HU Property Concierge:** two Mode B MCP connectors — `hu-properties-mcp` (Allow) + `hu-leads-mcp` (Require approval) on `bpaxqtxaysolzaeguwvg` ([ADR-049](../architecture/decisions/ADR-049.md)); setup `/home/bitnami/supabase/docs/digital-employees-setup.md`.
...
**AI backend**: Digital Employees Conversations API (`converse`) when the channel key starts with `mxde_…`; legacy HumaticAI RAGChat keys still work as fallback.
**Key Features**:
