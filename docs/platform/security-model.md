...
| S6 | **TRUNCATE still held on HRMS + MSA Hungary (+ Wave 2F drift on 9 other projects)** | HIGH | Weekly audits 2026-08-25 / **2026-09-01**: `TRUNCATE` for `anon`+`authenticated` on HRMS (42) and MSA Hungary (**64**). **Pipeline 2.0 RESOLVED 2026-08-25**. **2026-09-01:** nine additional projects never received Wave 2F — see **S10**. | Re-apply Wave 2F on HRMS + MSA Hungary; roll S10 template to FM, Comms, HU Storefront, Analytics+Stardom, Career Connect, Vacations Mgmt, lead-generator, Pipeline v1, HRMS Sandbox. See [security-audits/2026-09-01.md](security-audits/2026-09-01.md). |
...
| S10 | **Wave 2F TRUNCATE drift on 9 projects** | HIGH | Audit **2026-09-01**: FM (11), Comms (17), HU Storefront (19/20), Analytics+Stardom (50), Career Connect (3), Vacations Mgmt (17), matrix-lead-generator (7), Pipeline v1 (50), HRMS Sandbox (42). RLS does not apply to TRUNCATE. | Per-project Wave 2F (`REVOKE TRUNCATE … FROM anon, authenticated` + default privileges). Template: audit `proposed/s10_wave2f_template.sql`. |
| S11 | **MSA CY: SECURITY DEFINER view `opportunity_field_mismatches`** | MEDIUM | Advisors ERROR (new vs 2026-08-25). Same class as resolved S2. | `ALTER VIEW … SET (security_invoker = true)` or restrict SELECT grants. |
| S12 | **Pipeline 2.0: anon DML on 9 CRM tables** | MEDIUM | TRUNCATE fixed 2026-08-25 but anon INSERT/UPDATE/DELETE remain on commission/opportunity tables. | Wave 2 anon DML revoke (S9 pattern). |
| S13 | **SSO: anon DML on `sso_permission_load_failures`** | LOW | Audit **2026-09-01**: anon holds INSERT/UPDATE/DELETE on failure-log table. | Confirm EF-only write path; `REVOKE` from anon if not intentional. |
