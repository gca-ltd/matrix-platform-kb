...
| S6 | **TRUNCATE on MSA Hungary (HRMS portion closed)** | HIGH | **HRMS RESOLVED 2026-09-01** (holds **2026-09-16**). MSA Hungary still **66/66** TRUNCATE in audits **2026-09-15** / **2026-09-16** — deferred while app is in active testing. Other projects: see **S10**. Sandboxes are **out of standing contour** (2026-09-16). | Apply Wave 2F on MSA Hungary after test freeze; remaining S10 projects separately. See [security-audits/2026-09-16.md](security-audits/2026-09-16.md). |
| ~~S7~~ | ~~**Comms: RLS disabled on operational tables**~~ | RESOLVED (2026-08-25) | Migration `matrix-comms/supabase/migrations/20260825140000_s7_rls_ops_tables.sql`. Remaining Comms ERROR is `conversations_enriched` SECURITY DEFINER view — deferred; still ERROR in **2026-09-15** / **2026-09-16**. |
| S8 | **CY Web Site: anon EXECUTE on admin SECURITY DEFINER RPCs** | HIGH | Anon can call `update_user_role`, `remove_user_role`, `update_user_permission`, `handle_new_user_admin`, etc. via `/rest/v1/rpc/…`. Still **16 anon SECDEF EXECUTE** in audits **2026-09-01**, **2026-09-15**, **2026-09-16**. | `REVOKE EXECUTE … FROM PUBLIC, anon` (and tighten `authenticated` where needed); leave only intentional public RPCs. Proposed migration in audit `proposed/s8_cy_website_revoke_admin_rpc.sql`. |
| ~~S9~~ | ~~**CDL anon DML grants remain on ~30 tables**~~ | RESOLVED (2026-08-25) | Migration `matrix-platform-foundation/supabase/cdl/migrations/20260825142000_s9_anon_revoke_dml.sql`. Holds in **2026-09-01**, **2026-09-15**, **2026-09-16** (0 anon DML; 0 anon SECDEF EXECUTE). |
| S10 | **Wave 2F TRUNCATE drift (remaining standing-contour projects)** | HIGH | Audit **2026-09-16** standing contour: FM (11), Comms (17), Analytics+Stardom (50), Vacations Mgmt (17), matrix-lead-generator (7). Pipeline v1 / sandboxes **out of standing contour**. HU + Career Connect TRUNCATE hold. | Per-project Wave 2F after owners agree. Template: audit `proposed/s10_wave2f_template.sql`. |
| S11 | **MSA CY: SECURITY DEFINER view `opportunity_field_mismatches`** | MEDIUM | Advisors ERROR (still present **2026-09-15** / **2026-09-16**; WARN 158). Deferred — active testing. | `ALTER VIEW … SET (security_invoker = true)` after test freeze. |
| S12 | **Pipeline 2.0: anon DML on 9 CRM tables** | MEDIUM | TRUNCATE fixed 2026-08-25 but anon INSERT/UPDATE/DELETE remain (27 privileges / 9 tables in **2026-09-15** / **2026-09-16**). Deferred. | Wave 2 anon DML revoke (S9 pattern). |
| ~~S13~~ | ~~**SSO: anon DML on `sso_permission_load_failures`**~~ | RESOLVED (2026-09-01) | Holds **2026-09-15** / **2026-09-16** (0 anon DML). | `20260901122000_sso_anon_hygiene.sql`. |
| ~~S14~~ | ~~**HU Storefront: anon write hole on admin tables**~~ | RESOLVED (2026-09-01) | Holds **2026-09-15** / **2026-09-16**. Prod `sothebys-realty.hu` since 2026-09-09. | `20260901124000_s14_hu_anon_write_hole.sql`. |
| S15 | **HU Storefront: anon can UPDATE any `shared_collections` row** | HIGH | Policy `Anyone can update view count` is `UPDATE … USING (true) WITH CHECK (true)` for role `public`, and `anon` retains UPDATE. Still open **2026-09-16** (not in S18/S16 pass — needs UI). | Move counter behind SECDEF RPC / column-scoped policy; revoke anon UPDATE. |
| ~~S16~~ | ~~**HU Storefront: default privileges grant anon write on future tables**~~ | RESOLVED (2026-09-16) | Landed with S18: `ALTER DEFAULT PRIVILEGES FOR ROLE postgres IN SCHEMA public REVOKE INSERT, UPDATE, DELETE ON TABLES FROM anon`. Live `pg_default_acl` now `anon=rxtm`. Existing form grants unchanged. | `/home/bitnami/supabase` migration `20260916140000_s18_s16_conversion_sync_log.sql`. |
| S17 | **HU Storefront: any authenticated user can forge `audit_log` rows** | MEDIUM | Policy still `{public}` / `WITH CHECK (true)` + authenticated INSERT. Open **2026-09-16** (deferred — not in HU S18/S16 pass). | Scope policy to `service_role`; revoke authenticated INSERT once confirmed no client writes. |
| ~~S18~~ | ~~**HU Storefront (prod): `conversion_sync_log` RLS off + anon DML**~~ | RESOLVED (2026-09-16) | Advisors ERROR cleared. RLS on; `service_role`-only policy; `REVOKE ALL` from anon/authenticated. Table was empty; no in-repo/EF writer found (MSA `storefront-leads-ingest` uses service_role on `property_inquiries` only). Forms verified: anon INSERT privileges + INSERT policies hold; homepage 200; catalog SELECT 200; anon SELECT on journal → 42501. | Same migration as S16. Rollback: `supabase/rollback/20260916140000_s18_s16_conversion_sync_log.sql`. |
| S19 | **HRMS: anon can execute SECURITY DEFINER org-sync mutators** | HIGH | Confirmed still open **2026-09-16** (18 anon SECDEF; 4 mutators). Table grants hold. Out of this pass (HU-only remediations). | `REVOKE EXECUTE … FROM PUBLIC, anon` on the mutators. |
...
| ~~C13~~ | HU `conversion_sync_log` RLS off + anon DML (S18) + default ACL anon write (S16) | 2026-09-16 | `20260916140000_s18_s16_conversion_sync_log.sql` on `bpaxqtxaysolzaeguwvg`; Advisors ERROR cleared; forms grants unchanged |
